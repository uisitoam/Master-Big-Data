...
# Vamos a ver cómo funciona el algoritmo de descenso de gradiente
...
# Si queremos fijarle un número máximo de pasos
...
# Punto donde se alcanza el mínimo
pk
...
# Sin número máximo de pasos podemos usar while o repeat
dir <- deltax(pk[1],pk[2])
while(sum(dir^2)>tol){
pk <- pk+t*dir
dir <- deltax(pk[1],pk[2])
...
# Función recreando el algoritmo para elecciones de usuario
...
dir <- deltax(pk[1],pk[2],gamma)
if(sum(dir^2)<tol){break}
if(sum(dir^2)==Inf){break}
}

if((i<nitermax)&(sum(dir^2)<Inf)){
sol0=pk
}

if(i==nitermax){
sol0=list(sol=pk,aviso="Número máximo de iteraciones alcanzado")
}

if(sum(dir^2)==Inf){
sol0="La norma del gradiente va a infinito. Reduce el paso"
}
...
# Cambiando el valor de gamma (con paso fijo t=0.2)
graddec(0.1)
graddec(1)
graddec(10)
...
# Repetimos la función anterior pero con paso variable
...
f1 <- f(pk[1]+t*dir[1],pk[2]+t*dir[2],gamma)
f0 <- f(pk[1],pk[2],gamma)
