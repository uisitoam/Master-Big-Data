...
santiago = calculo("Santiago", [2200, 12950, [4500, 5000, 5500], [1100, 1150, 1300]], [50, 70, 80], [12, 14, 18], [2500, 3000])
vigo = calculo("Vigo", [1650, 12000, [4300, 4600, 4900], [1000, 1100, 1150]], [50, 60, 70], [11, 13, 16], [1500, 2000])
granada = calculo("Granada", [2300, 13250, [4500, 4800, 5200], [1100, 1150, 1300]], [60, 76, 90], [12, 15, 20], [2500, 3000])
sevilla = calculo("Sevilla", [3500, 15500, [5000, 5300, 5600], [1200, 1350, 1500]], [70, 80, 100], [13, 16, 20], [2500, 3000])
valencia = calculo("Valencia", [3250, 15500, [5200, 5500, 6000], [1200, 1300, 1400]], [70, 90, 110], [13, 16, 20], [2500, 3000])
alicante = calculo("Alicante", [1700, 12500, [4200, 4400, 4500], [1000, 1100, 1300]], [50, 65, 75], [13, 14, 18], [1750, 2000])
ll = calculo("LL", [1300, 12000, [3000, 3250, 3600], [1000, 1050, 1200]], [50, 60, 70], [10, 14, 18], [1500, 2000])
lpgc = calculo("LPGC", [1650, 12250, [3000, 3250, 3600], [1100, 1150, 1250]], [50, 65, 70], [10, 14, 18], [1500, 2000])
hermigua = calculo("Hermigua", [600, 8000, [1750, 2000, 2500], [800, 950, 1000]], [30, 45, 55], [8, 11, 15], [1000, 1250])
tazacorte = calculo("Tazacorte", [800, 10000, [2000, 2100, 2400], [900, 1000, 1100]], [40, 50, 65], [9, 12, 16], [1250, 1500])
valverde = calculo("Valverde", [650, 7000, [1400, 1500, 1700], [800, 900, 950]], [30, 40, 45], [8, 10, 14], [900, 1100])
cotillo = calculo("Cotillo", [1000, 10500, [2000, 2200, 2500], [850, 950, 1000]], [45, 60, 75], [10, 14, 17], [1300, 1500])
arrecife = calculo("Arrecife", [1000, 10500, [2000, 2100, 2300], [800, 900, 1100]], [45, 55, 65], [10, 14, 17], [1300, 1500])
...
def generar_valor_normal(minimo, medio, maximo):
    sigma = (maximo - minimo) / 6  # Aproximación asumiendo 99.7% de datos en +/- 3 sigma
...
def generar_ventas_platos(platos_info, total_ingresos):
    ...
    # Calcular ventas para cada plato
    ventas = {}
...
generar_archivos(
    "1", 1300, 12000,
    [3000, 3250, 3600], [1000, 1050, 1200],
    ll[0], [1500, 1700, 1800],
    [50*26, 60*26, 70*26], [90, 120, 150],
    [160, 290, 380], [30, 90, 130],
    mapa_canario
)

generar_archivos(
    "4", 1650, 12250,
    [3000, 3250, 3600], [1100, 1150, 1250],
    lpgc[0], [1300, 1700, 2000],
    [50*26, 65*26, 70*26], [90, 120, 160],
    [170, 300, 390], [30, 100, 140],
    mapa_canario
)

generar_archivos(
    "5", 800, 10000,
    [2000, 2100, 2400], [900, 1000, 1100],
    tazacorte[0], [950, 1100, 1300],
    [40*26, 50*26, 65*26], [40, 55, 70],
    [70, 80, 100], [15, 25, 40],
    mapa_canario
)
...
generar_archivos(
    "6", 650, 7000,
    [1400, 1500, 1700], [800, 900, 950],
    valverde[0], [400, 650, 800],
    [30*26, 40*26, 45*26], [20, 25, 40],
    [40, 60, 70], [5, 14, 23],
    mapa_canario
)
...
generar_archivos(
    "2", 600, 8000,
    [1750, 2000, 2500], [800, 950, 1000],
    hermigua[0], [500, 700, 850],
    [30*26, 45*26, 55*26], [30, 40, 60],
    [50, 90, 120], [10, 20, 35],
    mapa_canario
)
...
generar_archivos(
    "3", 1000, 10500,
    [2000, 2200, 2500], [850, 950, 1000],
    cotillo[0], [800, 1100, 1250],
    [45*26, 60*26, 75*26], [40, 60, 80],
    [80, 100, 150], [25, 30, 40],
    mapa_canario
)
...
generar_archivos(
    "7", 1000, 10500,
    [2000, 2100, 2300], [800, 900, 1100],
    arrecife[0], [800, 1150, 1200],
    [45*26, 55*26, 65*26], [35, 55, 75],
    [70, 105, 140], [23, 33, 40],
    mapa_canario
)
...
generar_archivos(
    "8", 2300, 13250,
    [4500, 4800, 5200], [1100, 1150, 1300],
    granada[0], [2800, 3200, 3500],
    [60*26, 76*26, 90*26], [310, 410, 600],
    [280, 410, 600], [150, 280, 400],
    mapa_andalucia
)

generar_archivos(
    "10", 2200, 12950,
    [4500, 5000, 5500], [1100, 1150, 1300],
    santiago[0], [2500, 2700, 3000],
    [50*26, 70*26, 80*26], [300, 400, 520],
    [270, 400, 590], [140, 275, 390],
    mapa_santiago
)

generar_archivos(
    "11", 1650, 12000,
    [4300, 4600, 4900], [1000, 1100, 1150],
    vigo[0], [1800, 2000, 2250],
    [50*26, 60*26, 70*26], [150, 185, 215],
    [200, 280, 320], [80, 85, 100],
    mapa_santiago
)

generar_archivos(
    "12", 1700, 12500,
    [4200, 4400, 4500], [1000, 1100, 1300],
    alicante[0], [1900, 2100, 2300],
    [50*26, 65*26, 75*26], [160, 185, 230],
    [210, 300, 330], [90, 100, 110],
    mapa_valencia
)

generar_archivos(
    "9", 3500, 15500,
    [5000, 5300, 5600], [1200, 1350, 1500],
    sevilla[0], [2500, 2800, 3000],
    [70*26, 80*26, 100*26], [340, 435, 650],
    [260, 390, 580], [130, 260, 390],
    mapa_andalucia
)

generar_archivos(
    "13", 3250, 15500,
    [5200, 5500, 6000], [1200, 1300, 1400],
    valencia[0], [2600, 2900, 3150],
    [70*26, 90*26, 110*26], [350, 440, 660],
    [280, 410, 600], [150, 275, 400],
    mapa_valencia
)
...
def generar_valoraciones_csv(id_restaurante, clientes, cliente_dom):
    ...
    while fecha_actual < fin:
        ...
        # Generar las fechas según la condición (6 días seguidos, 1 día de descanso)
        dias = []
        dia = 1
        ...
        total_dias = len(dias)
        # Distribuir valoraciones presenciales
...
min_clientes_1 = [50, 30, 45, 50, 40, 30, 45, 60, 70, 50, 50, 50, 70]
min_clientes = [idx * 26 for idx in min_clientes_1]
